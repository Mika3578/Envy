...
- [x] Removed `register` keyword (`Envy/Buffer.cpp`, `HashLib/TigerTree.cpp`, `Envy/Strings.cpp`, `Envy/QueryHashTable.cpp`, `Envy/QueryHashGroup.cpp`, `Envy/MatchObjects.cpp`, `Envy/XML.cpp`, `Envy/DlgLanguage.cpp`, `Envy/PageSettingsSkins.cpp`) - reserved in C++17 / C5033; remaining first-party uses are tracked under #84
- [x] Replaced `throw()` with `noexcept` (Envy/Buffer.{h,cpp}, Connection.h, HostCache.h `CHostCacheList` inlines)
