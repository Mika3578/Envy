...
.DESCRIPTION
  Default (-Quick): Format-oriented fast checks + EnvyTests Release x64 + Envy Release x64.
  -Full: also Envy Release Win32 (and EnvyTests Win32 when the project builds there).
...
$testsX64 = Join-Path $root 'tests\Release x64\EnvyTests.exe'
if (-not (Test-Path -LiteralPath $testsX64)) {
	$testsX64 = Join-Path $root "tests\$Configuration x64\EnvyTests.exe"
}
