...
- **PR #294 Copilot re-request helper** — Temporary `request-copilot-review.yml` gated to pull request #294 only (Actions `GITHUB_TOKEN` can `requestReviews` when the agent integration cannot). Removes any existing Copilot request before re-requesting (`union:true` alone does not emit `ReviewRequestedEvent` / start a fresh review on a new HEAD). Optional `workflow_dispatch`. Delete after #294 lands.
