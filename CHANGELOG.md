...
### Fixed
- **Interop harness exchange labels + cross-packet evidence (#160, #294)** — Kad HELLO/PING/FIND_NODE use direction-specific labels (req+res both required); packet-evidence aggregates labels across transport-tagged `from-pcap-tcp-<stream>-*` / `from-pcap-udp-*` files; TCP reassembly joins only within the same `tcp.stream` (UDP never joined); `source_exchange` accepts SourceEx or SourceEx2 pairs; negative `pcap_duration_sec` rejected in Python config and `run-live.ps1`; malformed labeled evidence FAILs.
