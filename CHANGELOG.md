...
### Fixed
- **VersionChecker HTTP response size cap (#81/#82)** — Automatic version-check downloads use `LimitContentLength(VERSION_CHECK_HTTP_RESPONSE_MAX)` (64 KiB) and fail-closed oversize bodies (`VersionCheckerHttpResponseOk`) before key=value parse.
- **Update Servers HTTP response size cap (#81/#82)** — `CUpdateServersDlg` calls `LimitContentLength(UPDATE_SERVERS_HTTP_RESPONSE_MAX)` (32 MiB) and fail-closes oversize bodies before MET/hublist import (`UpdateServersHttpResponseOk`). `CHttpRequest` now always discards the response when the content-length cap is reached (no truncated buffer treated as complete; closes chunked `nMore==0` race).
