...
### Fixed
- **PR Gate superseded check-runs** — A `cancelled` required check is pending until a newer generation appears or the existing timeout elapses. Concurrency cancel of an older Code Quality/Build/Security/CodeQL wave no longer false-fails PR Gate before the replacement `Format Check` (or other context) is registered. `cancelled` is never treated as success; `failure`/`timed_out`/`action_required`/`startup_failure`/`stale` still fail immediately.
