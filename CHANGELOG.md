...
### Fixed
- **Debug assert opening an already-shared folder** — `CLibraryFolders::GetFolder` now takes `Library.m_pSection` itself (same as `IsFolderShared`), so Debug `ASSUME_LOCK` no longer fires in `GetFolder` / `GetFolderIterator` / `GetNextFolder` / `GetFolderByPath` when the caller omitted the lock. `OpenPath` still holds the lock around `Display`. `OnCollection` uses the `AddFolder` result instead of an unlocked `GetFolder` overwrite.
