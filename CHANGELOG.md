...
### Added
- **Cloud Agent Linux dev environment (`.cursor/environment.json`)** — Cloud Agents now boot a ready-to-use Linux environment for Envy's cross-platform checks. The install step provisions `clang-format`/`clang-tidy`/`cppcheck` and installs the Remote web UI security test dependencies (`npm --prefix Remote/tests ci`). The MFC/ATL app and HashLib remain Windows-only (MSVC v145); this environment targets the same checks CI runs on `ubuntu-latest`, with `cppcheck` available as an additional local analysis tool.
