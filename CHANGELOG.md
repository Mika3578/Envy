...
### Fixed
- **QuickStart wizard reliability** — Debug first-run no longer asserts when flag artwork is missing (`CFlags` empty state + `WizardProfilePage` text fallback). Connection speeds use a strict kbps/Mbps/Gbps parser (rejects junk/overflow; no `CString::Find` unit test). Upload cap keeps `(100 - FreeBandwidthFactor)%` of `OutSpeed` with 64-bit math (the old integer `/ 100` term was 0 for normal factors). The Connection page no longer bootstraps ED2K/G1/G2/DC or calls `Network.Connect` before network selection. DC hublist download runs only when Direct Connect is enabled. Listen port policy is 1024–65535 (matches the error text). Helpers/tests: `Envy/WizardQuickStartPolicy.h`, `Envy/FlagsState.h`, `tests/test_quickstart_wizard_policy_smoke.cpp`. ADC/ADCS is still **not** advertised. 6-page UX modernization is a follow-up.
