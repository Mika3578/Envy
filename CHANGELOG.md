...
### Fixed
- **Stability slice (#92)** — Remote Base64 encode no longer writes padding into an empty buffer (null/zero-length returns empty); CHM metadata `LocalAlloc` locale buffer is always `LocalFree`'d; TorrentEnvy finished-page clipboard allocates `(length+1)*sizeof(WCHAR)` for a wide NUL. EnvyTests cover Base64 empty/null and RFC vectors. Remaining #92 items (EDClients lock order, `TerminateThread`) deferred.
- **Remote CSRF on query-key mutations (#77)** — `CheckCookie` now requires a CSRF token when mutating query keys are present (`connect`/`disconnect`, `filter_set`, download-group select/deselect/exclusive, upload queue expand/collapse), not only when those words appear in the URL path. Non-empty `_method` no longer skips CSRF. Classic Remote HTML links/forms carry `csrf_token` for those actions.
- **Network listen no longer waits for NAT (#141)** — `CNetwork::OnRun` opens TCP/UDP listeners as soon as the app is live, instead of stalling on `UPnPFinder::IsAsyncFindRunning()`. `MapPorts()` starts only after a successful bind/listen; UPnP tier retries and refreshes no longer `continue` past the listen path. Aligns with D-011 (interface → listen → async NAT → external ports).
- **UPnP SSDP interface selection** — MiniUPnPc discovery binds to the Internet-facing IPv4 via `GetAdaptersAddresses`/`GetBestRoute2` and performs targeted IGD discovery in a single MiniUPnPc discovery receive phase (`upnpDiscoverDevices(..., searchalltypes=1)`), instead of sequential legacy ST searches. MiniUPnPc 2.0 does not enforce a strict wall-clock SSDP deadline (`UPnPTimeout` is the per-receive delay); absolute SSDP/HTTP timeout bounding is deferred to #142 / P3. Only explicit IGD/WAN advertisements become direct mapping candidates; root-device fallback is restricted to the selected gateway and duplicate LOCATION values are removed by exact string match. Non-IGD responders fail cleanly without WAN mapping commands.
- **WebHook IE plugin registration (E_ACCESSDENIED)** — Startup no longer attempts to register `WebHook32.dll` / `WebHook64.dll` (and historical `WebHook.dll`) when `Downloads.WebHookEnable` is false (default). When enabled, per-user registration writes the IE Browser Helper Objects key under HKCU; HKLM is used only for explicit machine-wide `DllRegisterServer`. Shared x86/x64 CLSID unchanged. This is a legacy Internet Explorer BHO (not an HTTP webhook; Edge Chromium does not load it); `envy://url:` remains the useful handoff path.
- **Search panel DPI layout** — Search Input/Advanced task boxes lay out labels, edits, size combos, protocol rows, and buttons from font metrics and `SCALE()` (progressive Y), instead of historical fixed pixel offsets. `CBS_DROPDOWN` still reserves a scaled drop-down list height while vertical stacking advances by the visible edit height only. Box preferred heights are applied from `CSearchPanel::UpdateBoxHeights()` after create/skin change (not from `OnSize`). Keeps the two-column Advanced layout and the `#135` Search panel width floor unchanged.
- **Skin engine P0 geometry / metrics** — `Settings.Skin.StatusbarHeight` no longer aliases `MenubarHeight` on load/save. `CSkinWindow::ParseRect` reads `size` (not `point`) for `point`+`size` anchors; part-name `.HDA` truncation ignores `FindOneOf == -1`. Invalid `roundRect` `size` is rejected instead of using uninitialized radii. `LoadFromXML` aggregates section failures (unknown root / invalid manifest also fail the file); load is still non-transactional (no rollback of earlier sections). Skin `<option>` metrics use strict parse + Settings-aligned clamps via `SkinEngineP0.h` (non-numeric keeps the current value). EnvyTests smoke coverage in `test_skin_engine_p0_smoke.cpp`. No HiDPI/coordinate-system change.
- **Search panel minimum width** — `CSearchWnd` now uses `max(SidebarWidth, SCALE(200))` for the left search panel only (Shareaza `PANEL_WIDTH` heritage). Prevents `CSearchAdvancedBox` two-column protocol/size layout from collapsing when `Settings.Skin.SidebarWidth` is stored below ~200 logical px; other sidebars keep the shared setting (PeerProject skins ~182 px remain valid elsewhere).
- **CoolMenu selection double-band** — Owner-draw popup highlight no longer paints a second blue strip under the selected item when item height (`SCALE(23)`) exceeds the `ButtonMap.MenuSelect` / `CCoolMenu.Hover` asset height (24px). `CImages::DrawButton` / `DrawButtonMap` stretch one state vertically instead of tiling; `CCoolMenu::OnDrawItemInternal` keeps `rcItem` clipped to `DRAWITEMSTRUCT` (removed `m_bPrinted` bottom expansion) and scales icon gutter offsets with `SCALE()`.
...
- **ED2K upload queue RTTI** — `CUploadQueue::StartImpl` no longer uses `dynamic_cast` to reach `CUploadTransferED2K` (invalid under `/GR-`). The construction path (`CEDClient::OnQueueRequest`) guarantees the type; the call uses a documented `static_cast` and still connects via `CEDClient::Connect()`. No wire-format change.
- **ED2K shared-file serialization crash** - `CEDPacket::WriteFile` used `dynamic_cast<const CLibraryFile*>` on the complete-file path, but Envy is built without RTTI (`/GR-`), so `CEDNeighbour::OnIdChange -> SendSharedFiles -> WriteFile` crashed in `__RTDynamicCast` (`std::__non_rtti_object`) on every valid `CLibraryFile`. Restored the historical `static_cast` for `bPartial == false`; all such callers (`CEDNeighbour::SendSharedFiles`, `CEDClient` view-shared-dir answer) statically pass `CLibraryFile*`, and partial-file callers never take the cast. No RTTI or compiler-option change; serialized ED2K bytes are unchanged.
- **Startup splash step count** — When `eDonkey.EnableKad` is on (default since #125), `InitInstance` runs an extra `SplashStep("Kademlia DHT")` that was never counted in `nSplashSteps` (still 21 from the pre-Kad path). Debug builds hit `ASSERT(m_nPos < m_nMax)` in `CSplashDlg::Step` on the following `"Upgrade Manager"` step. `nSplashSteps` now includes the conditional Kad step.
- **ED2K EnableKad persistence (#124)** — Register `eDonkey.EnableKad` with `Settings.Add` (default `true`, matching the settings reference). Without registration the flag stayed at static zero-init (`false`), so `InitKademlia()` never ran even when `EnableKadHello` / `KadFindValue` defaulted on. Policy helpers in `Envy/Ed2kKadSettingsPolicy.h` with smoke tests. Does not wire Kad hits into `AddSourceED2K` (follow-up).
...
### Changed
- **ED2K Hello/HelloAnswer golden vectors** — EnvyTests now freeze deterministic TCP Hello (`0x01`) and HelloAnswer (`0x4C`) byte layouts via `Ed2kHelloWire.h`, including MiscOptions1/2 honesty bits (Ext Multipacket / SecureIdent / AICH / CryptLayer / Kad nibble = 0). Compression advertise nibble remains frozen for a later advertise-vs-implement decision. No protocol or runtime change.
...
### Security
- **CSPRNG fail-closed for security tokens (#78)** — Session IDs, CSRF tokens, and Remote password salts no longer fall back to `rand()`/`GetTickCount()` seeding. Shared helper `Envy/SecureRandom.h` uses `BCryptGenRandom` (`BCRYPT_USE_SYSTEM_PREFERRED_RNG`) with optional `CryptGenRandom` only when a CryptoAPI provider was acquired; RNG failure aborts session creation and salt hashing. G2 QueryKeys, BitTorrent UDP tracker transaction IDs, ED2K UDP status keys, and BitTorrent peer-ID padding use the same CSPRNG. Wire formats unchanged. PBKDF2 (#79), CSRF policy (#77), and XSS (#76) remain separate.
- **ED2K Hello capabilities honesty (#87)** — Stop advertising AICH in `CEDClient::SendHello` until C2C AICH request/answer handlers exist. Pack FeatureVersions / MiscOptions2 via `Envy/Ed2kHelloCapabilities.h` (smoke-tested). CryptLayer SUPPORTS/REQUESTS/REQUIRES remain 0 pending a separate TCP-obfuscation interop audit (packet PUBLICKEY crypto is not treated as equivalent). SecureIdent advertisement stays 0 (#75/#110).
