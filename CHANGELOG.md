...
- **BitTorrent source-response double-free (#92)** — `CDownloadTransferBT::OnSourceResponse` no longer `delete`s `pPacket->m_pNode` when `peers` is missing/non-list (packet owns the node); null-guard `pRoot` before use.
