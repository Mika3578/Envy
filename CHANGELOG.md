...
### Fixed
- **CBuffer Inflate/Ungzip default output cap (#81)** — `Inflate`/`Ungzip` with `nMaxOutput=0` now apply `CBUFFER_INFLATE_MAX` (32 MiB) via `Decompress2` / growth clamps (`CBufferInflateOutputOk`); closes unlimited GGEP/datagram/legacy inflate zip-bomb while callers may still pass an explicit smaller cap.
