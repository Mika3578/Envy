...
- **CBuffer UnBZip output cap (#81)** — `UnBZip(nMaxOutput)` stops bzip2 growth at 32 MiB (`CBUFFER_UNBZIP_MAX`); DC hublist import and file-listing loaders reject oversized compressed input and call the capped path (`CBufferUnBZipInputOk` / `CBufferUnBZipOutputOk`).
