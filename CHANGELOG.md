...
### Fixed
- **Interop harness fail-closed Kad evidence (#160, #294)** — Kad HELLO/PING/FIND_NODE require minimum body shapes; Kad hits inside ED2K TCP frame spans are ignored; evidence JSON uses fixed `source_name`; `optional_pcap` PASSes only after usable stopped capture (early capture exit FAILs). SEARCH_RES uses production `<SenderID 16><TargetID 16><Count 2>`; raw `.bin` dumps stay out of sanitized trees; `capability_negotiation` uses Hello compare path (not generic label PASS).
