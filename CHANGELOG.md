...
### Changed
- **SonarCloud New Code version marker (#234)** — `.sonarcloud.properties` sets `sonar.projectVersion=4.2.0` (aligned with `Envy.rc` ProductVersion) so Automatic Analysis can record a VERSION event and stop treating the 2026-05–modernization window as unbounded New Code. A follow-up version bump is required after the marker analysis before develop QG can reflect only post-marker changes. See `docs/10_dev/sonarcloud-exclusions.md`.
