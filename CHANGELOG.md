...
### Added
- **Uploads Fair-Use (10% media)** — `Uploads.FairUseMode` is opt-in and **implemented**: each remote IPv4 client is clipped to 10% of an audio/video library file. The host+path ledger reserves on accept and charges body bytes as they are written; unused reservation rolls back on close so HTTP HEAD / aborted transfers do not burn the quota. Checkbox enabled and DDX-bound. HTTP 416 / ED2K skip / DC `$Error` when the host already received its share. BitTorrent and partials are not limited. Mapping: `docs/50_user/transfer-settings.md`.
