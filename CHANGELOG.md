...
### Fixed
- **BitTorrent SourcesWanted re-enforced for PEX/source-exchange/tracker (#81)** — Stop `AddSourceBT` / source-list growth once effective count reaches `Settings.Downloads.SourcesWanted` in `OnUtPex`, `OnSourceResponse`, tracker HTTP source apply, and UDP announce peer list (`BtSourcesWantedAllowsMore` / `BtCompactPeerListBytesOk`).
