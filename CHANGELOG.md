...
### Fixed
- **G1 QueryHit QHD XML size fail-closed (#81)** — Reject attacker-controlled `nXMLSize` that does not leave room for the trailing GUID (`G1QueryHitXmlFits`); replace soft clamp to 0 and the commented-out size check with `AfxThrowUserException` (same class as invalid public-size).
