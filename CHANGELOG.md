...
- **NMDC hub text encoding (#224)** — Decode/encode NMDC user text with an explicit hub code page (`Settings.DC.CodePage`, `0` = Windows ACP; per-host override `CHostCacheHost::m_nCodePage`) via `DcNmdcText.h`, instead of forcing `UTF8Decode` on RX and `CP_ACP`/`WriteStringUTF8` on TX. Covers hub chat/`$HubName`/`$MyINFO`/`$To` private messages/`$SR` search hits. Stops mojibake and HostCache pollution. ADC/ADCS unchanged. EnvyTests cover ASCII, UTF-8, CP1251, wrong-charset contrast, length, and fixtures.
