...
### Changed
- **Repository governance** — Streamlined agent rules (`AGENTS.md`, `.cursor/rules/`, tool pointers): GitHub Issues/Projects/Milestones/PRs are the shared work source of truth; volatile material belongs in gitignored `.local/` and `references/`; documentation updates required only when durable project knowledge changes.
