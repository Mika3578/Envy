...
### Fixed
- **G2 SGP UDP reassembly / inflate zip-bomb (#81)** — Cap SGP fragment count (64), reassembled bytes (256 KiB), and `Inflate(G2_SGP_INFLATE_MAX)` in `CDatagramIn::ToG2Packet` / `OnReceiveSGP` so UDP G2 cannot bypass TCP `MaximumPacket` via multi-fragment deflate.
