...
- **Bootstrap catalogues (2026-09-18)** — Refresh `Data/DefaultServices.dat` / `Data/DefaultServers.dat`: drop static ED2K IPs and dead GWC/hublists; HTTPS `server.met` (eMule-Security, shortypower) and DC hublists (dchublist.org/ru, Team Elite); gtk-gnutella UHCs; Transmission/libtorrent DHT routers. `CDHT::Connect` cold-start sends BEP 5 `find_node` to HostCache BitTorrent hosts instead of a hard-coded DNS name. Kad remote `nodes.dat` is **not** added (Kad2 remains partial). `SmartUpgrade` migrates only the obsolete persisted defaults `https://peerates.net/servers.met` and `http://dchublist.com/hublist.xml.bz2`. See `docs/30_protocols/bootstrap-sources.md`.
