...
### Fixed
- **Interop harness fail-closed Kad evidence (#160, #294)** — Kad HELLO/PING/FIND_NODE require minimum body shapes; Kad hits inside ED2K TCP frame spans are ignored; evidence JSON uses fixed `source_name`; `optional_pcap` PASSes only after usable stopped capture (early capture exit FAILs).
