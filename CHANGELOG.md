...
### Security
- Fixed CodeQL `cpp/invalid-pointer-deref` findings in `Envy/Images.cpp` (`#2546`, `#2545`, `#2544`, `#2543`) by adding null/handle/object-size guards and failure-safe bitmap ownership handling in `CImages::BlendAlpha` and `CImages::PreBlend`, while preserving existing alpha blending and pre-multiplication behavior.
