...
- **Bootstrap catalogues (2026-09-18)** — Refresh `Data/DefaultServices.dat` / `Data/DefaultServers.dat`: drop static ED2K IPs and dead GWC/hublists; HTTPS `server.met` (eMule-Security, shortypower) and DC hublists (dchublist.org/ru, Team Elite); gtk-gnutella UHCs; Transmission/libtorrent DHT routers. `CDHT::Connect` cold-start pings HostCache BT catalogue hosts instead of a hard-coded DNS name. Kad remote `nodes.dat` is **not** added (Kad2 remains partial). See `docs/30_protocols/bootstrap-sources.md`.
