...
### Changed
- **CI audit + micro-optimizations (2026-09)** — Documented measured PR/push timings and critical path in `docs/10_dev/CI_AUDIT_2026-09.md`. Skip empty NuGet restore in `windows-msbuild` (~22s/job measured no-op), upload PR build logs only on failure, always emit required `Documentation Check` (classify no-op when out of scope), PR Gate poll interval 10s, and record live Protect develop approval-count drift vs docs.
