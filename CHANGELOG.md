...
### Fixed
- **eDonkey partial import blocked the main window and lied about copy completion** — `CDonkeyImportDlg` is modeless; `CEDPartImporter` no longer writes `IDC_LOG` from the worker; `IDS_ED2K_EPI_COPY_FINISHED` is logged after `dtaskMergeFile` actually ends; `CDownload::GetProgress()` exposes merge-task progress; `RunMergeFile` advances by copied bytes, not per-fragment jumps.
...
- **Review lifecycle — Copilot is the final reviewer** — `Review Lifecycle Gate` requires the latest HEAD review from exact login `copilot-pull-request-reviewer` to be `APPROVED` (`REVIEW_GATE_ADVISORY=false`); live `reviewRequests` keep the gate pending. Other AI tools are advisory pre-review. Evaluator loads from base/default (same-repo HEAD bootstrap only until the gate lands on `develop`). `request-copilot-review.yml` requests Copilot only after PR Gate succeeds (`workflow_run` + `workflow_dispatch`), never on every push. PR Gate also waits for required `gitleaks` and `SonarCloud Code Analysis` so that request cannot race those checks. Do not enable Copilot `review_on_push`. #299 bootstraps Copilot manually until that helper exists on `develop`.

### Added
- **Modeless eDonkey partial-import jobs** — Queue + per-job stage/progress UI (`PartialImportTypes.h`). One disk merge at a time. Hide does not cancel. Duplicate ED2K still rejected (complementary fragment merge is a follow-up). Smoke: `tests/test_partial_import_workflow_smoke.cpp`.
