...
- **ED2K COMPRESSEDPART stream inflate cap (#81)** — `OnCompressedPart` / `OnCompressedPart64` share `AcceptCompressedPartChunk` to reject uncompressed output past one ED2K part (or remaining file size) via `Ed2kCompressedPartInflateOk` before `SubmitData`; closes streaming zip-bomb into the download.
