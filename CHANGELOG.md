...
### Added
- **Uploads Fair-Use (10% media)** — `Uploads.FairUseMode` is opt-in and **implemented**: each remote IPv4 client is clipped to 10% of an audio/video library file. Checkbox enabled and DDX-bound. HTTP 416 / ED2K skip / DC `$Error` when the host already received its share. BitTorrent and partials are not limited. Mapping: `docs/50_user/transfer-settings.md`.
