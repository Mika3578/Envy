...
### Added
- **Crashpad vs Sentry Native evaluation (#90, D-017)** — Isolated `tools/crash-probe` (MSVC v145, x64 + Win32 static triplets) plus maintainer audit `docs/10_dev/crashpad-vs-sentry-native.md`. Capture engine preference is Crashpad (upload off by default). Sentry Native is Option B only. Homemade MiniDump (PR #243) is not the product engine. Root `vcpkg.json` is unchanged.
