...
### Security
- **NMDC `$ADCGET`/`$ADCSND` numeric parse (#81)** — Strict full-token unsigned decimal offset/length (`DcAdcGetValidate.h`). `$ADCGET` length may be exactly `-1` (until-EOF); `$ADCSND` forbids `-1`. Fixed-length replies must match the request (no silent `min()` truncate in `CDownloadTransferDC::OnDownload`). EnvyTests smoke coverage.
