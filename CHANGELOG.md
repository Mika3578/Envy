...
### Fixed
- **Interop harness exchange labels + cross-packet evidence (#160, #294)** — Kad HELLO/PING/FIND_NODE use direction-specific labels (req+res both required); packet-evidence aggregates labels across transport-tagged `from-pcap-tcp-<stream>-*` / `from-pcap-udp-*` files; TCP reassembly joins only within the same `tcp.stream` (UDP never joined); extractors dispatch by transport tag; `PUBLICIP_REQ` empty-body check; COMPRESSEDPART allows partial chunks; Hello `userhash` redacted in attachable evidence; missing configured evidence paths FAIL; `source_exchange` accepts SourceEx or SourceEx2; negative `pcap_duration_sec` rejected in config/`run-live.ps1`/dry-run; malformed labeled evidence FAILs.
...
- **PR #294 Copilot re-request helper** — Temporary `request-copilot-review.yml` gated to pull request #294 only (Actions `GITHUB_TOKEN` can `requestReviews` when the agent integration cannot). Clears bot reviewers via `requestReviews(union:false, botIds:[])` then re-requests Copilot so a fresh `ReviewRequestedEvent` is emitted (`union:true` alone is a no-op when Copilot is already requested). Optional `workflow_dispatch`. Delete after #294 lands.
