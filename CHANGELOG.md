...
### Security
- Fixed CodeQL `cpp/use-after-free` alert family by making preview/index/list reallocation paths in `CtrlSearchDetailPanel`, `HubHorizon`, and `HashDatabase` allocate/copy into temporary storage before replacing old buffers, eliminating stale-pointer use while preserving runtime behavior and on-disk/protocol semantics.
  - Updated `docs/DEVELOPMENT_PLAN.md` with a one-line follow-up entry tracking the completed stale-pointer remediation and remaining verification posture for the affected preview/index/list buffer paths.
