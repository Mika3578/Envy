...
### Fixed
- **NMDC hub text encoding (#224)** — Decode/encode NMDC user text with an explicit hub code page (`Settings.DC.CodePage`, `0` = Windows ACP; per-host override `CHostCacheHost::m_nCodePage`) via `DcNmdcText.h`, instead of forcing `UTF8Decode` on RX and `CP_ACP` via `WriteString` on TX. Stops mojibake in chat/`$HubName`/`$MyINFO` and HostCache pollution. ADC/ADCS unchanged. EnvyTests cover ASCII, UTF-8, CP1251, wrong-charset contrast, length, and fixtures.
