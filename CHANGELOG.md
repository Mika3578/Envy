...
### Fixed
- **Bencode recursion depth + integer overflow (#82 partial)** — `CBENode::Decode` rejects nesting deeper than 32 (`BencodeDepthOk`). Bencode integers/lengths use overflow-safe `ParseInt64Bounded` (also backs `atoin`). Remaining #82 items (`ut_metadata` caps, ED2K `.met` blob bounds, hashset payload length) are still open.
