...
### Added
- **IPv6 / HTTPS recovery docs (no code)** — Canonical Shareaza gap audit, proposed D-020 (`CEnvyAddress` endpoint; reject dual `IN_ADDR`/`IN6_ADDR` APIs) and D-021 (Schannel for download TLS; keep WinINet for catalogues). IPv6 plan refreshed: `Settings.Connection.EnableIPv6` does not exist; `IPv6Support.*` is not compiled. Transfer HTTPS remains absent (`https://` still remapped to HTTP:80 in current code). Tests for future work stay local/offline. Root CMake gitignore `Testing/` narrowed to `/Testing/` so `docs/40_quality/testing/` is not ignored on Windows. See `docs/20_arch/IPV6_HTTPS_RECOVERY_SERIES.md`.
