...
### Added
- **ED2K live interop harness (#160, phase 1)** — Opt-in `python3 tools/interop/run.py` records ENVY ↔ eMule Community / aMule evidence with PASS/FAIL/SKIP/NOT_IMPLEMENTED, isolated profiles, sanitized artifacts, and Envy self-golden Hello parse. Live binaries and the public P2P network are **not** required CI. This does **not** claim ENVY is fully interoperable. See `tools/interop/README.md`.
