...
- **Local crash minidumps (#90)** — First-party Windows crash reporting replaces BugTrap. Debug and Release write privacy-conscious minidumps plus a sanitized `.txt` sidecar under `%LOCALAPPDATA%\Envy\CrashReports\`. The next launch can copy the text, open the folder, or open GitHub’s issue page. Dumps are never uploaded. See `docs/50_user/crash-reports.md` and `docs/10_dev/crash-reporting.md`.
