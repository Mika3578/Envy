...
- **Bootstrap catalogues (2026-09-18)** — Refresh `Data/DefaultServices.dat` / `Data/DefaultServers.dat`: drop static ED2K IPs and dead GWC/hublists; HTTPS `server.met` (eMule-Security, shortypower) and DC hublists (dchublist.org/ru, Team Elite); gtk-gnutella UHCs; Transmission/libtorrent DHT routers. `CDHT::Connect` still uses `router.bittorrent.com` when HostCache has no node IDs (same catalogue name; Latin-1 `BTPacket.cpp` vs UTF-8 Format Check). Kad remote `nodes.dat` is **not** added (Kad2 remains partial). See `docs/30_protocols/bootstrap-sources.md`.
