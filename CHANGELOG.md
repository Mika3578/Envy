...
### Changed
- **CI CodeQL + Format gates** — Every PR to `develop` always runs CodeQL `Analyze (c-cpp)`, `Analyze (javascript-typescript)`, and `Analyze (csharp)` so Code Scanning no longer reports missing configurations. Format Check fails the job on clang-format diffs (no `continue-on-error`); no relevant C/C++ changes → success no-op. PR Gate always waits for those four checks.
