...
### Fixed
- **ED2K .met blob + hashset payload bounds (#82)** — `CEDTag::Read(CFile*)` rejects TAG_BLOB lengths over 4 MiB or past EOF before allocation. Hashset answers require payload length exactly `nBlocks * 16` (`Ed2kHashsetPayloadFits`) before `SetHashset`. Completes the remaining file-backed / hashset items from #82 (bencode/ut_metadata shipped separately).
