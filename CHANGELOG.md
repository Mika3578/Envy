...
### Fixed
- **G2 HIT_WRAP embedded G1 length fail-closed (#81)** — Reject negative / oversize wrapped `GNUTELLAPACKET::m_nLength` via `G1WrappedPayloadFits` in `SeekToWrapped` and `G1WrappedPayloadLengthOk` before `CG1Packet::New` `(DWORD)` cast/`Write` (avoids ~4 GiB allocation from `m_nLength == -1`).
