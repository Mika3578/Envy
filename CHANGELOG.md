...
### Fixed
- **G2 compound sub-packet length overflow (#81)** — Overflow-safe `G2SubpacketPayloadFits` / `G2FrameLengthFits` replace wrap-prone `remaining < body + prefix` checks in `CG2Packet::ReadPacket`, `SkipCompound`, and `ReadBuffer`.
