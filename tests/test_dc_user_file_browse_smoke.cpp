...
static std::vector< std::string > ParseNicks(const char* psz, size_t n = 0)
{
	...
	if ( psz == NULL )
	{
		DcParseNickList( NULL, 0, [ & ]( const char*, size_t ) { return TRUE; } );
		return o;
	}
	if ( n == 0 )
		n = strlen( psz );
...
{
	return ParseNicks( "" ).empty()
		&& ParseNicks( NULL ).empty()
		&& ParseNicks( "$$" ).empty();
}
...
{
	return DcNickListPayloadOk( DC_NICKLIST_PAYLOAD_MAX ) == TRUE
		&& DcNickListPayloadOk( DC_NICKLIST_PAYLOAD_MAX + 1 ) == FALSE
		&& DcParseNickList( "x", DC_NICKLIST_PAYLOAD_MAX + 1,
			[]( const char*, size_t ) { return TRUE; } ) == FALSE;
}
...
{
	std::string sUrl;
	return DcFormatFileListUrl( "", "10.0.0.1", 411, sUrl ) == FALSE
		&& DcFormatFileListUrl( "alice", "bad host", 411, sUrl ) == FALSE
		&& DcFormatFileListUrl( "alice", "10.0.0.1", 0, sUrl ) == FALSE
		&& DcFormatFileListUrl( "alice", "10.0.0.1", 70000, sUrl ) == FALSE;
}

static bool test_filelist_download_name()
{
	return DcIsFileListDownloadNameW( L"files.xml.bz2" ) == TRUE
		&& DcIsFileListDownloadNameW( L"files.xml" ) == TRUE
...
{
	const std::string sXml = std::string( "<FileListing Version=\"1\">" )
		+ FileTag( "a.mp3", "1", kTth ) + "</FileListing>";
	std::vector< DcFileListEntry > o;
	if ( DcParseFileListingXml( sXml.c_str(), sXml.size(), o ) != dcFileListOk )
		return false;
	return o.size() == 1 && o[ 0 ].sName == "a.mp3" && o[ 0 ].nSize == 1 && o[ 0 ].sTth == kTth
		&& o[ 0 ].sPath.empty();
}

static bool test_filelist_nested_dir()
{
	const std::string sXml = std::string( "<FileListing Version=\"1\"><Directory Name=\"FolderA\">" )
		+ FileTag( "file1.ext", "10", kTth )
		+ "</Directory><Directory Name=\"FolderB\">"
		+ FileTag( "file3.ext", "3", kTth )
		+ "</Directory></FileListing>";
	std::vector< DcFileListEntry > o;
	if ( DcParseFileListingXml( sXml.c_str(), sXml.size(), o ) != dcFileListOk )
		return false;
	return o.size() == 2
		&& o[ 0 ].sPath == "FolderA" && o[ 0 ].sName == "file1.ext"
		&& o[ 1 ].sPath == "FolderB" && o[ 1 ].sName == "file3.ext";
}

static bool test_filelist_several_files()
{
	const std::string sXml = std::string( "<FileListing Version=\"1\"><Directory Name=\"D\">" )
		+ FileTag( "a.bin", "1", kTth )
		+ FileTag( "b.bin", "2", kTth )
		+ "</Directory></FileListing>";
	std::vector< DcFileListEntry > o;
...
{
	const std::string sXml = std::string( "<FileListing Version=\"1\">" )
		+ FileTag( "a.mp3", "1", "NOT_A_VALID_TTH____________" ) + "</FileListing>";
	std::vector< DcFileListEntry > o;
...
{
	const std::string sNeg = std::string( "<FileListing Version=\"1\">" )
		+ FileTag( "a.mp3", "-1", kTth ) + "</FileListing>";
	const std::string sJunk = std::string( "<FileListing Version=\"1\">" )
		+ FileTag( "a.mp3", "12x", kTth ) + "</FileListing>";
	std::vector< DcFileListEntry > o;
	return DcParseFileListingXml( sNeg.c_str(), sNeg.size(), o ) == dcFileListBadSize
		&& DcParseFileListingXml( sJunk.c_str(), sJunk.size(), o ) == dcFileListBadSize;
}

static bool test_filelist_truncated_xml()
{
	const std::string sXml = std::string( "<FileListing Version=\"1\">" )
		+ FileTag( "a.mp3", "1", kTth );
	std::vector< DcFileListEntry > o;
...
static bool test_filelist_malformed_xml()
{
	...
	std::vector< DcFileListEntry > o;
	return DcParseFileListingXml( psz, strlen( psz ), o ) == dcFileListMalformed
		&& DcParseFileListingXml( "", 0, o ) == dcFileListEmpty;
}

static bool test_filelist_missing_name()
{
	const std::string sXml = std::string( "<FileListing Version=\"1\"><File Size=\"1\" TTH=\"" )
		+ kTth + "\"/></FileListing>";
	std::vector< DcFileListEntry > o;
...
{
	const std::string sXml = std::string( "<FileListing Version=\"1\"><Directory Name=\"..\">" )
		+ FileTag( "a.mp3", "1", kTth ) + "</Directory></FileListing>";
	std::vector< DcFileListEntry > o;
	if ( DcParseFileListingXml( sXml.c_str(), sXml.size(), o ) != dcFileListTraversal )
		return false;
	const std::string sSlash = std::string( "<FileListing Version=\"1\">" )
		+ FileTag( "a/b.mp3", "1", kTth ) + "</FileListing>";
...
{
	if ( DcFileListEntryCountOk( DC_FILELIST_ENTRIES_MAX - 1 ) != TRUE
		|| DcFileListEntryCountOk( DC_FILELIST_ENTRIES_MAX ) != FALSE )
		return false;
	DWORD n = 0;
	std::string sXml = "<FileListing Version=\"1\">";
	for ( int i = 0; i < 3; ++i )
...
static bool test_filelist_uncompressed_cap()
{
	...
	if ( DC_FILELIST_BYTES_MAX != kMax )
		return false;
	return DcFileListUncompressedOk( 1 ) == TRUE
		&& DcFileListUncompressedOk( kMax ) == TRUE
		&& DcFileListUncompressedOk( kMax + 1 ) == FALSE
		&& DcFileListCompressedOk( 1 ) == TRUE
		&& DcFileListCompressedOk( kMax + 1 ) == FALSE
		&& DcFileListUncompressedOk( 0 ) == FALSE;
}

static bool test_filelist_tth_predicate()
{
	return DcFileListTthOk( L"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" ) == TRUE
		&& DcFileListTthOk( L"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" ) == TRUE
		&& DcFileListTthOk( L"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" ) == FALSE
		&& DcFileListTthOk( L"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=" ) == FALSE
		&& DcFileListTthOk( L"" ) == FALSE;
}
