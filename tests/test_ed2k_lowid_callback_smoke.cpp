...
{
	return Ed2kPublicIpReqPayloadOk( 0 ) == TRUE
		&& Ed2kPublicIpReqPayloadOk( 1 ) == FALSE;
}

static bool test_publicip_answer_exact_4()
{
	return Ed2kPublicIpAnswerPayloadOk( 4 ) == TRUE
		&& Ed2kPublicIpAnswerPayloadOk( 0 ) == FALSE
		&& Ed2kPublicIpAnswerPayloadOk( 1 ) == FALSE
		&& Ed2kPublicIpAnswerPayloadOk( 3 ) == FALSE
		&& Ed2kPublicIpAnswerPayloadOk( 5 ) == FALSE;
}
...
static bool test_publicip_answer_truncated_decode()
{
	...
	return Ed2kPublicIpAnswerDecode( raw, 3, &nIp ) == FALSE
		&& Ed2kPublicIpAnswerDecode( raw, 0, &nIp ) == FALSE;
}
...
{
	return Ed2kC2cCallbackExactBytes() == 38u
		&& Ed2kC2cCallbackPayloadOk( 38 ) == TRUE
		&& Ed2kC2cCallbackPayloadOk( 34 ) == FALSE
		&& Ed2kC2cCallbackPayloadOk( 0 ) == FALSE
		&& Ed2kC2cCallbackPayloadOk( 39 ) == FALSE;
}
...
static bool test_callback_parse_valid_and_truncations()
{
	...
	raw[32] = 0x01; raw[33] = 0x02; raw[34] = 0x03; raw[35] = 0x04;	// IP LE
	raw[36] = 0x36; raw[37] = 0x12;				// port 0x1236 LE
...
{
	return Ed2kCallbackEndpointOk( 0x04030201u, 0 ) == FALSE
		&& Ed2kCallbackEndpointOk( 0, 4662 ) == FALSE
		&& Ed2kCallbackEndpointOk( 0xFFFFFFFFu, 4662 ) == FALSE
		&& Ed2kCallbackEndpointOk( 5u, 4662 ) == FALSE	// LowID-shaped
...
static bool test_callback_empty_file_hash()
{
	BYTE zeros[16] = {};
	BYTE nonzero[16] = { 1 };
	return Ed2kCallbackFileHashNonEmpty( zeros ) == FALSE
		&& Ed2kCallbackFileHashNonEmpty( nonzero ) == TRUE;
}
...
{
	return Ed2kLowIdConnectUsesServerPush( 12345u ) == TRUE
		&& Ed2kLowIdConnectUsesServerPush( 0x04030201u ) == FALSE;
}
