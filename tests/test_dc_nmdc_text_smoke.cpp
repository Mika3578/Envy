...
{
	return DcResolveNmdcCodePage( 0 ) == CP_ACP
		&& DcResolveNmdcCodePage( 1251 ) == 1251
		&& DcResolveNmdcCodePage( CP_UTF8 ) == CP_UTF8;
}
...
static bool test_dc_nmdc_wrong_charset_not_silent_utf8()
{
	...
	return asLegacy == L"\x041F\x0440\x0438\x0432\x0435\x0442"
		&& asUtf8 != asLegacy;
}

static bool test_dc_nmdc_explicit_length_ignores_trailer()
{
	const char kBuf[] = "AB\xCF\xF0""XX";
...
{
	const char kBuf[] = "x";
	return DecodeNmdcText( kBuf, 0, 1251 ).empty()
		&& DecodeNmdcText( nullptr, 5, 1251 ).empty()
		&& EncodeNmdcText( L"", 1251 ).empty()
		&& EncodeNmdcText( nullptr, 1251 ).empty();
}
...
static bool test_dc_nmdc_chat_fixture_cp1251()
{
	...
	return msg.find( L"\x0411\x043E\x0442>" ) == 0
		&& msg.find( L"\x041F\x0440\x0438\x0432\x0435\x0442" ) != std::wstring::npos;
}
...
static bool test_dc_nmdc_ascii_regression_unchanged()
{
	...
	return wAcp == L"Public Hub" && wUtf == L"Public Hub"
		&& EncodeNmdcText( L"Public Hub", 0 ) == kHub;
}
