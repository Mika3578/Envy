...
{
	fwprintf(stderr,
	         L"CrashProbe --init-only | --crash av|heap|stack|fastfail|terminate|invalid|multithread\n"
	         L"  [--database DIR] [--handler crashpad_handler.exe] [--no-handler]\n");
}
...
static void CrashInvalidParameter()
{
	_invalid_parameter_noinfo();
}

static DWORD WINAPI CrashThreadProc(LPVOID)
{
	CrashAccessViolation();
	return 0;
}

static void CrashMultithread()
{
	HANDLE threads[3];
	for (int i = 0; i < 3; ++i)
		threads[i] = CreateThread(nullptr, 0, CrashThreadProc, nullptr, 0, nullptr);
	CrashAccessViolation();
...
static int RunCrash(const wchar_t* kind)
{
	if (_wcsicmp(kind, L"av") == 0)
		...
		CrashFastFail();
	else if (_wcsicmp(kind, L"terminate") == 0)
		CrashTerminate();
	else if (_wcsicmp(kind, L"invalid") == 0)
		CrashInvalidParameter();
	else if (_wcsicmp(kind, L"multithread") == 0)
		CrashMultithread();
	else
	{
...
int wmain(int argc, wchar_t** argv)
{
	...
	for (int i = 1; i < argc; ++i)
	{
		if (wcscmp(argv[i], L"--init-only") == 0)
			initOnly = true;
		else if (wcscmp(argv[i], L"--no-handler") == 0)
			noHandler = true;
	...
	if (noHandler)
	{
		fwprintf(stderr, L"no-handler: skipping Crashpad init\n");
		if (crashKind == nullptr)
			return 1;
