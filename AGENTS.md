...
application. Optional CMake under `HashLib/` is auxiliary only
...
CMake into a full app or tests build outside a PR dedicated to Phase 5.
...
- **Don't** reintroduce a root CMake app build, or present `HashLib/`
  CMake (or a restored orphan `tests/CMakeLists.txt`) as equivalent to
  building Envy / EnvyTests, outside a PR explicitly dedicated to Phase 5
  CMake work. Do not modify `Visual Studio/Envy.sln` solely to satisfy CMake.
