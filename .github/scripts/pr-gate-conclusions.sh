...
# Live Protect develop integration_id values (ruleset 16457466). Default 15368
# is GitHub Actions; gitleaks and SonarCloud use their GitHub Apps.
protect_develop_app_id = {
	"gitleaks": 57789,
	"SonarCloud Code Analysis": 12526,
}


def expected_app_id(check_name):
	return protect_develop_app_id.get(check_name, 15368)
...
while idx < len(text):
	...
	for run in runs:
		...
		if not isinstance(app, dict):
			raise SystemExit("check-runs response included an invalid check_run app")
		app_id = app.get("id")
