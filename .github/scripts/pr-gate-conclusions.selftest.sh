...
expect() {
	...
	if [[ "$got" -ne "$want_rc" ]]; then
		echo "FAIL $name: $fn($arg) rc=$got want=$want_rc" >&2
		fail=1
	else
		echo "OK   $name"
	fi
...
if [[ "$cancelled_state" == "pending" && "$elapsed" -ge "$timeout_sec" && "$cancelled_state" != "ok" ]]; then
	...
	echo "FAIL cancelled-alone-times-out-without-pass state=$cancelled_state" >&2
	fail=1
fi
...
for run in runs:
	if not isinstance(run, dict):
		raise SystemExit("fixture check_run must be an object")
	if "app" not in run:
		name = run.get("name", "")
		if name == "gitleaks":
			run["app"] = {"id": 57789}
...
got="$(select_conc "$HEAD_A" "$json_t3" "$CHECK_FORMAT")"
expect_out select-cancelled-then-success "${TSV_COMPLETED_SUCCESS}${HEAD_A}" printf '%s' "$got"
...
json_old_fail_new_success="$(check_json '[
  ...
  {"id":2,"name":"secret-scan","status":"completed","conclusion":"success","head_sha":"'"$HEAD_A${JSON_STARTED_AT_1805_SUFFIX}"'
]')"
got="$(select_conc "$HEAD_A" "$json_old_fail_new_success" "secret-scan")"
expect_out select-old-failure-new-success "${TSV_COMPLETED_SUCCESS}${HEAD_A}" printf '%s' "$got"
...
got="$(select_conc "$HEAD_A" "$json_t2" "$CHECK_FORMAT")"
...
json_old_success_new_queued="$(check_json '[
  ...
  {"id":2,"name":"gitleaks","status":"queued","conclusion":null,"head_sha":"'"$HEAD_A${JSON_STARTED_AT_1805_SUFFIX}"'
]')"
...
json_old_success_new_progress="$(check_json '[
  {"id":1,"name":"gitleaks","status":"completed","conclusion":"success","head_sha":"'"$HEAD_A"'","started_at":"2026-09-21T18:00:00Z"},
  {"id":2,"name":"gitleaks","status":"in_progress","conclusion":null,"head_sha":"'"$HEAD_A${JSON_STARTED_AT_1805_SUFFIX}"'
]')"
...
json_old_success_new_cancelled="$(check_json '[
  ...
  {"id":2,"name":"Documentation Check","status":"completed","conclusion":"cancelled","head_sha":"'"$HEAD_A${JSON_STARTED_AT_1805_SUFFIX}"'
]')"
...
expect_out select-latest-generation "${TSV_COMPLETED_SUCCESS}${HEAD_A}" printf '%s' "$got"
...
expect_out select-current-merge-sha "${TSV_COMPLETED_SUCCESS}${MERGE_A}" printf '%s' "$got"
...
json_head_and_merge="$(check_json '[
  ...
  {"id":80,"name":"Documentation Check","status":"completed","conclusion":"success","head_sha":"'"$MERGE_A${JSON_STARTED_AT_1805_SUFFIX}"'
]')"
got="$(select_conc "$HEAD_A" "$json_head_and_merge" "Documentation Check" "$MERGE_A")"
expect_out select-latest-across-current-head-and-merge "${TSV_COMPLETED_SUCCESS}${MERGE_A}" printf '%s' "$got"

# Wrong GitHub App id must not satisfy a required context name.
json_wrong_app="$(check_json '[
  {"id":1,"name":"gitleaks","status":"completed","conclusion":"success","head_sha":"'"$HEAD_A"'","app":{"id":15368}},
...
if [[ "$malformed_rc" -eq 0 ]]; then
	echo "FAIL malformed-json-fails" >&2
	fail=1
else
...
if [[ "$malformed_run_rc" -eq 0 ]]; then
	echo "FAIL malformed-check-run-fails-closed" >&2
	fail=1
else
...
for control_case in name status conclusion head_sha; do
  ...
  {"id":1,"name":"Format Check","status":"completed","conclusion":"success","head_sha":"'"$HEAD_A"'\tspoof"}
		...
		;;
	*)
		echo "FAIL unknown control-char case: $control_case" >&2
		fail=1
		continue
		;;
	esac
	...
	if [[ "$control_char_rc" -eq 0 ]]; then
		echo "FAIL control-char-$control_case-fails-closed" >&2
		fail=1
	else
...
got="$(select_conc "$HEAD_A" "$json_t0" "$CHECK_FORMAT")"
...
if [[ "$(classify_gate_outcome completed cancelled false)" == "ok" ]]; then
	echo "FAIL regression-t1-never-pass-on-cancelled" >&2
	fail=1
else
...
got="$(select_conc "$HEAD_A" "$json_t2b" "$CHECK_FORMAT")"
...
got="$(select_conc "$HEAD_A" "$json_t3b" "$CHECK_FORMAT")"
expect_out regression-t3-select-success "${TSV_COMPLETED_SUCCESS}${HEAD_A}" printf '%s' "$got"
...
set -e
if [[ "$rc" -eq 0 ]]; then
	echo "FAIL capture_checked did not fail on false" >&2
	fail=1
else
...
if [[ "$*" != *"filter=all"* ]]; then
	echo "expected check-runs filter=all in gh api call: $*" >&2
	exit 2
fi
cat <<EOF_JSON
{"check_runs":[
  {"id":100,"name":"Lint build files","status":"completed","conclusion":"cancelled","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}},
  {"id":101,"name":"secret-scan","status":"completed","conclusion":"success","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}},
  {"id":102,"name":"Vcpkg manifest sanity","status":"completed","conclusion":"success","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}},
  {"id":103,"name":"Analyze (c-cpp)","status":"completed","conclusion":"success","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}},
  {"id":104,"name":"Analyze (javascript-typescript)","status":"completed","conclusion":"success","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}},
  {"id":105,"name":"Analyze (csharp)","status":"completed","conclusion":"success","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}},
  {"id":106,"name":"Format Check","status":"completed","conclusion":"success","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}},
  {"id":107,"name":"Documentation Check","status":"completed","conclusion":"success","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}},
  {"id":110,"name":"gitleaks","status":"completed","conclusion":"success","head_sha":"${STUB_HEAD_SHA}","app":{"id":57789}},
  {"id":111,"name":"SonarCloud Code Analysis","status":"completed","conclusion":"success","head_sha":"${STUB_HEAD_SHA}","app":{"id":12526}},
  {"id":108,"name":"Build x64 Release","status":"completed","conclusion":"skipped","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}},
  {"id":109,"name":"Build Win32 Release","status":"completed","conclusion":"skipped","head_sha":"${STUB_HEAD_SHA}","app":{"id":15368}}
]}
EOF_JSON
EOF_GH
...
if [[ "$timeout_rc" -eq 0 ]]; then
	echo "FAIL cancelled-check-e2e-timeout: gate unexpectedly succeeded" >&2
	fail=1
