...
| Crashpad | vcpkg `crashpad` (baseline `2026-07-02`) | Out-of-process Windows crash capture (`crashpad_handler.exe`) | Windows-only (not EnvyCore) | Active | Medium | vcpkg pin + Crash Probe | Apache-2.0; upload URL empty; `SetUploadsEnabled(false)` |
