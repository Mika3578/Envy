...
| bzip2 (vcpkg) | vcpkg | Compression | Portable | Owner TBD / Active | Low | vcpkg cadence | |
| OpenSSL (vcpkg) | vcpkg | Crypto | Portable | Owner TBD / Active | Medium | vcpkg + CVE review | Prefer for new core paths when practical vs Windows-only APIs |
...
| BugTrap (vendored) | `Services/BugTrap` | Crash reporting | **Windows-only** | Owner TBD / Legacy | Medium | Replacement track (#90) | Do not treat as EnvyCore dependency |
...
| gtest (vcpkg feature) | vcpkg `tests` feature | Unit tests | Portable | Active for HashLib | Low | vcpkg | |
