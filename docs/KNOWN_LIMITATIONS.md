...
- **ED2K/Kad live interop:** Opcode-level documentation is not a substitute for Envy ↔ eMule Community / aMule transfer tests. Live interoperability remains **unverified**. Opt-in harness: `tools/interop/README.md` + Windows `OPERATOR_CHECKLIST.md` (#160). Harness preparation is not an interop claim. Required CI never launches reference clients or depends on the public P2P network.
