...
- **Root CMake removed (2026-09):** Root `CMakeLists.txt` / `CMakePresets.json` and orphan `tests/CMakeLists.txt` were deleted as incomplete scaffolding. Authoritative build is `Visual Studio/Envy.sln`. Optional HashLib-only CMake remains under `HashLib/CMakeLists.txt`.
