...
- CI workflows exist and the full-solution build is green on `windows-2025-vs2026`. The `.github/settings.yml` `required_status_checks` contexts now match the names emitted on `pull_request` runs (`Build x64 Release`, `Build Win32 Release`, `Lint build files`, `Analyze (c-cpp)`, `Analyze (javascript-typescript)`). The Debug legs (`Build x64 Debug` / `Build Win32 Debug`) build only on pushes to integration branches, not on PRs, so they are intentionally excluded from the required contexts to avoid blocking every PR on a check that never reports. **Maintainer action required:** apply these contexts to the live `Protect develop` ruleset to make the build a mandatory merge gate (an assistant cannot change live branch protection).
