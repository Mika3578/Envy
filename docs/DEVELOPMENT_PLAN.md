...
- **Changelog Entry:** 2026-09-19 — NMDC hub user list + remote file-list browse: bounded `$NickList` merge with `$MyINFO`, hub-user Browse via existing `CHostBrowser` `PROTOCOL_DC`, fail-closed `files.xml.bz2` parse, `$ADCGET file files.xml.bz2`, skip unhashed TTH on outgoing lists. Browse Host left tree is filled from FileListing directories; completion is hub+port not nick-only. ADC hubs / TLS / `$MyINFO` slots remain separate. Live hub interop unverified.
- **Changelog Entry:** 2026-09-19 — NMDC hub user list + remote file-list browse: bounded `$NickList` merge with `$MyINFO`, hub-user Browse via existing `CHostBrowser` `PROTOCOL_DC`, fail-closed `files.xml.bz2` parse, `$ADCGET file files.xml.bz2`, skip unhashed TTH on outgoing lists. Browse Host left tree is filled from FileListing directories **before** `CNetwork` takes hit ownership; empty directories still appear; directories count toward the 100k node cap. Completion is hub+port not nick-only. ADC hubs / TLS / `$MyINFO` slots remain separate. Live hub interop unverified.
