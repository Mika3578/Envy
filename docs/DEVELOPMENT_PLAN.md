...
- **Changelog Entry:** 2026-09-16 — Publishable surface: root `LICENSE`, drop `.github/agents/my-agent.md`, MSBuild-vs-CMake clarity, `version.json` SoT docs, DEPENDENCIES vcpkg/Services note. Do **not** restore orphan `tests/CMakeLists.txt` (needs parent HashLib target; Phase 5 later). Prior commits on this branch already fixed AGENTS after metadata cleanup.
