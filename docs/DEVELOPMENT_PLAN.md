...
- **Changelog Entry:** 2026-09-21 — Repository tree hygiene inventory (synced after #299 `27f9942`): 3350 files / 164 dirs on this branch; pre-#299 snapshot was 3344 on `0f39cd3`. No mass moves; canonical docs map + roadmap Current State dedupe. Copilot #304: PR summary now matches live approval count **1**. See `docs/10_dev/repository-tree-hygiene.md`.
...
- **Changelog Entry:** 2026-09-20 — Review Lifecycle Gate advisory mode: `PENDING` (no HEAD Copilot review yet) does not fail the Actions job. Set `REVIEW_GATE_ADVISORY=false` when making the check required. Canonical: `docs/10_dev/POST_MERGE_REVIEW_AUDIT_2026-09.md`.
- **Changelog Entry:** 2026-09-20 — Copilot re-request helper: `pull_request_target` (no checkout), queue instead of cancel-in-progress, restore reviewers on failure. Gate still uses `pull_request` because it runs PR scripts. Canonical: `docs/10_dev/POST_MERGE_REVIEW_AUDIT_2026-09.md`.
- **Changelog Entry:** 2026-09-20 — Review lifecycle gate Copilot round: paginate `latestOpinionatedReviews` fail-closed; pending on Copilot `review_requested`; drop unresolved-thread fail (native ruleset owns it; Actions cannot subscribe to resolution); Copilot approval off until UI settings are verified. Canonical: `docs/10_dev/POST_MERGE_REVIEW_AUDIT_2026-09.md`.
- **Changelog Entry:** 2026-09-20 — Review lifecycle gate: skip drafts (Copilot does not review them, so fail-closed on draft made #299 look broken); fail-closed GraphQL/PR-number collection; concurrency group falls back to `github.run_id`. Canonical: `docs/10_dev/POST_MERGE_REVIEW_AUDIT_2026-09.md`.
- **Changelog Entry:** 2026-09-20 — Review lifecycle gate: HEAD-bound CI check so develop PRs are not treated as merge-ready while Copilot Code Review of the current SHA is still running. Protect develop has no `copilot_code_review` rule (`VERIFIED VIA API`); thread resolution is not enough. Gate is not ruleset-required until validated. Canonical: `docs/10_dev/POST_MERGE_REVIEW_AUDIT_2026-09.md`.
...
- **Changelog Entry:** 2026-09-20 — #160/#294 Copilot round 4: per-packet tshark chunks (no flatten), SEARCH_RES dual layout (eMule 34 + Envy outbound 17), capability needs Hello+HelloAnswer, .bin skip only on captures sanitize.
- **Changelog Entry:** 2026-09-20 — #160/#294 Copilot round 3: SEARCH_RES production layout (34-byte header), exclude .bin from sanitized captures, capability_negotiation Hello-compare path only.
