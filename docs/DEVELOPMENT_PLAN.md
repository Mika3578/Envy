...
- **Changelog Entry:** 2026-09-19 — Debug `ASSUME_LOCK` cascade on shared-folder open: `CLibraryFolders::GetFolder` now locks `Library.m_pSection` itself (like `IsFolderShared`); `OpenPath` still holds the lock for `Display`; `OnCollection` no longer overwrites `AddFolder` with an unlocked `GetFolder`.
