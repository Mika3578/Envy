...
| Direct Connect | `Envy/DC*` | [NMDC](https://nmdc.sourceforge.net/NMDC.html), [ADC](https://adc.sourceforge.io/ADC.html), [ADC-EXT](https://adc.sourceforge.io/ADC-EXT.html) | Preserve NMDC. Hub user list + remote `files.xml.bz2` browse is wired (`CHostBrowser` `PROTOCOL_DC`); live hub interop unverified. ADC/ADCS hub protocol is **not implemented** (separate future layer). NMDC `ADCGet`/`ADCSND` ≠ ADC hubs. |
