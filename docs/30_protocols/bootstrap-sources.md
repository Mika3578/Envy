...
| BitTorrent DHT | implemented (bootstrap) | `DefaultServers.dat` `B` DNS routers loaded into HostCache; `CDHT::Connect` pings those hosts when no node IDs are cached. Successful nodes persist in `HostCache.dat`. |
