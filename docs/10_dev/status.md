...
- **NMDC hub users + file-list browse:** `$GetNickList` is still sent after `$Hello`. `$NickList` is a bounded nick-only seed merged into the hub session’s `m_oUsers` (identity is **hub + nick**). `$MyINFO` remains the metadata path; `$Quit` still removes users. Hub chat can Browse a selected remote user through existing `CHostBrowser` `PROTOCOL_DC` (`files.xml.bz2`). Incoming lists are fail-closed; FileListing directories populate the Browse Host left tree **before** `CNetwork` owns the hit chain (G2 physical/virtual packets are unused on this path; empty directories still appear). Outgoing lists skip files without Tiger. This is **not** “full DC++ support”. Live hub tests are not part of CI.
