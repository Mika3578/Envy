...
- Live Envy ↔ eMule Community / aMule transfers remain **unverified**. Opt-in harness (`tools/interop/`, issue #160, harness v1.1) records production vs harness vs evidence states and prepares Windows live runs; Cursor Cloud validates Python dry-run/self-test only. The harness is evidence infrastructure, not an interoperability claim. Public P2P availability is never required PR CI.
...
- Wire handlers present for bootstrap, HELLO, PING/PONG, REQ/RES (FIND_NODE), SEARCH_KEY/SOURCE/RES, PUBLISH_KEY/SOURCE/RES, **FIREWALLED_REQ/RES/ACK** — **partial**:
  ...
  - **TCP firewall-detection baseline (#86 phase 1):** `KADEMLIA_FIREWALLED_REQ` (0x50, exact 2-byte TCP port) / `KADEMLIA_FIREWALLED_RES` (0x58, exact 4-byte observed IPv4) / inbound `KADEMLIA_FIREWALLED2_REQ` (0x53, min 19 bytes) / UDP `FIREWALLED_ACK_RES` (0x59) and ED2K `OP_KAD_FWTCPCHECK_ACK` (0xA8) count toward TCP Open. Bounded outbound checks (max 4 verified contacts) gathered from the XOR zone tree via `ForEachContact` (not the retired bucket array). Public IP requires two independent agreeing peers. TCP and UDP firewall states are distinct; UDP remains Unknown. Inbound REQ is rate-limited; probe IP is always the UDP source (port from payload only). Connect-back TCP is recorded, not executed. Does **not** flip `Network.IsFirewalled()`. Helpers + EnvyTests: `Envy/KadFirewallCheck.h`, `tests/test_kad_firewall_check_smoke.cpp`.
  - **Routing-table maintenance (focused slice):** `Envy/KadRoutingTable.h` replaces the fixed 128-bucket array with an aMule/eMule-style XOR zone tree (`CanSplit`: level < 127 and size == K and (`prefixInteger` < KK or level < KBASE); K=10). Zone identity is a 128-bit XOR `zonePrefix` (not a uint32 `zoneIndex`) so depth ≥ 32 never shifts a 32-bit index. Unsplittable full leaves use LRU + a 1-slot replacement cache (verified healthy contacts are not evicted by unverified candidates; stale/dead incumbents can be replaced). `CKademlia::OnTimer` runs bounded small-timer pings and at most one stale-zone FIND_NODE refresh per cycle (first big timer ~10s after a zone is observed, then 1h interval, 10s global gap). Valid outstanding FIND_NODE_RES (full contact-list size + matching TargetID) / PONG from a known endpoint calls `ObserveAliveByEndpoint` (liveness, not `verified`). Unsolicited HELLO_RES is ignored; `verified` is set only after an outstanding HELLO_REQ. Subnet caps: 2 contacts per IPv4 `/24` per leaf, 10 globally, 1 Kad ID per IP (LAN excepted when `allowLan`). nodes.dat bits are not live-verified. FindContact returns a copy. Tests: `tests/test_kad_routing_table.cpp`.
  - **App-trigger source search:** ED2K downloads call `CKademlia::SearchSource` from `FindMoreSources` / automatic starve paths when `EnableKad` is on (`DownloadWithSearch::MaybeSearchKadSources`). Outbound `KADEMLIA2_SEARCH_SOURCE_REQ` includes little-endian FileSize. Helpers + EnvyTests: `Envy/KadSearchSourceRequest.h`, `tests/test_kad_search_source_request_smoke.cpp`.
...
- Cold-start: `HostCache::Load` calls `CheckMinimumServers(PROTOCOL_KAD)` if the Kad list is empty. `DefaultServers.dat` still ships **no** `K` rows. When the Kad cache is empty, Envy imports a local `nodes.dat` from `Settings.General.DataPath` (and, if present, a fresh eMule/aMule `config\nodes.dat`). `CHostCache::ImportNodes` parses legacy v0 plus new-format **v1 / v2 / v3** via `Envy/KadNodesDat.h` (v3 edition 1 is bootstrap-only: XOR-closest 50 contacts, not the whole 500-1000 pool). UDP-key and `verified` bytes are consumed and discarded — runtime UDP-key protocol is **not implemented**. `CKademlia::Bootstrap()` still reads `HostCache.Kademlia` (up to 5 BOOTSTRAP_REQ, 30 s throttle) and does **not** dump bootstrap candidates into routing leaves. If the cache is still empty it logs that no bootstrap contacts were found. Remote HTTP `nodes.dat` discovery is a follow-up, not a completeness claim.
- **Absent:** UDP firewall tester (`FWCHECKUDPREQ` / `KADEMLIA2_FIREWALLUDP`), Buddy, Kad callback, UDP keys, notes search/publish. Routing split/LRU/refresh and eclipse `/24` limits are implemented locally (not live-verified).
- **Kad TCP firewall-detection + routing-table maintenance baselines implemented; UDP firewall verification, Buddy and callback remain incomplete.** Do not say Kad firewall or routing support is complete.
- ED2K Hello Kad version nibble is advertised as **0** (`CEDClient::SendHello`) even when Kad is enabled — honest until Buddy/UDP firewall and live interop are verified.
- `docs/30_protocols/kad/kad2-compatibility-report.md` records opcode/format matching plus local routing-table maintenance; it is **not** live DHT interop.
