...
- Wire handlers present for bootstrap, HELLO, PING/PONG, REQ/RES (FIND_NODE), SEARCH_KEY/SOURCE/RES, PUBLISH_KEY/SOURCE/RES, **FIREWALLED_REQ/RES/ACK** — **partial**:
  ...
  - **Routing-table maintenance (focused slice):** `Envy/KadRoutingTable.h` replaces the fixed 128-bucket array with an aMule/eMule-style XOR zone tree (`CanSplit`: level < 127 and size == K and (`zoneIndex` < KK or level < KBASE); K=10). Unsplittable full leaves use LRU + a 1-slot replacement cache (verified healthy contacts are not evicted by unverified candidates; stale/dead incumbents can be replaced). `CKademlia::OnTimer` runs bounded small-timer pings and at most one stale-zone FIND_NODE refresh per cycle (1h zone interval, 10s global gap). Valid FIND_NODE_RES / PONG from a known endpoint calls `ObserveAliveByEndpoint` (liveness, not `verified`). Subnet caps: 2 contacts per IPv4 `/24` per leaf, 10 globally, 1 Kad ID per IP (LAN excepted when `allowLan`). `verified` means HELLO_RES (or `MarkContactVerified`) only — candidate listings and nodes.dat bits are not live-verified. FindContact returns a copy. Tests: `tests/test_kad_routing_table.cpp`.
